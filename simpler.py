...
print output_file
        #a = subprocess.Popen(['PATH=$PATH'], bufsize=1, stdout=output_file, stderr=output_file, stdin=output_file, shell=True)
##        proc  = subprocess.Popen(['echo', '$PATH'], bufsize=1, stdout=output_file, stderr=output_file, stdin=output_file, shell=True)
        #a = subprocess.call(['$PATH'], bufsize=1, stdout=output_file, stderr=output_file, stdin=output_file, shell=True)
for d in sys.path:
    print d
print "\n"
sys.stdout.flush()
...
#            proc = subprocess.call([ffmpegEXE, '-nostats', '-i', adjusted_MP, '-filter_complex', 'ebur128=peak=true+sample:framelog=verbose', '-f', 'null', '-'], bufsize=1, stdout=output_file, stderr=output_file, stdin=output_file, shell=True)
            sys.stdout.flush()
            output_file.close()
            if os.path.isfile(adjusted_file):
